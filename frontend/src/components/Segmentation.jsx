...
export default function Segmentation({ devices = [], availableSegments = [], segmentScaffold = null, selectedDevices = new Set(), setSelectedDevices, handleBulkLimit, handleBulkSegment, handleLimitForIps }) {
  ...
  const scaffoldSegments = segmentScaffold?.segments || [];
  ...
  return (
    <div>
      <div className="mb-4">
        <h1 className="text-3xl font-bold text-white mb-1">Segmentation & Limits</h1>
        <p className="text-gray-400">Macro segmentation overview and per-device limit adjustments.</p>
      </div>
      <div className="grid grid-cols-1 xl:grid-cols-12 gap-2 mb-4">
        <div className="xl:col-span-8 bg-gray-900/40 border border-gray-800 rounded-xl p-2.5 md:p-3">
          <div className="flex flex-col gap-3 lg:flex-row lg:items-end lg:justify-between">
            <div>
              <div className="flex flex-wrap gap-2 mb-2">
                ...
                <span className="px-3 py-1 rounded-full bg-purple-500/10 text-purple-200 border border-purple-500/20 text-xs">{scaffoldSegments.length} scaffold segments</span>
              </div>
          ...
          {scaffoldSegments.length > 0 && (
            <div className="mt-4 rounded-xl border border-gray-800 bg-gray-950/30 p-3">
              <div className="text-xs uppercase tracking-[0.24em] text-gray-500 mb-2">Scaffold preview</div>
              <div className="flex flex-wrap gap-2">
                {scaffoldSegments.slice(0, 8).map((seg) => (
                  <span key={seg.name} className="px-2 py-1 rounded-full border border-gray-700 bg-gray-900/40 text-xs text-gray-200">
                    {seg.name || 'unknown'} · VLAN {seg.vlan_id ?? '—'} · {seg.device_count ?? 0}
                  </span>
                ))}
              </div>
            </div>
        ...
        <div className="xl:col-span-4 space-y-2">
          <div className="bg-gray-900/40 border border-gray-800 rounded-xl p-2.5 md:p-3">
            ...
            <p className="text-xs text-gray-400 mb-4">Review selected devices before applying changes.</p>
              ...
              <div className="rounded-xl bg-gray-950/30 border border-gray-800 p-2">
