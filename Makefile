...
test-enforcement:
	...
	@sudo bash scripts/enforcement_test.sh 2>&1 | sudo tee -a /var/log/zerotrust-enforcement-test.log
...
cron-install:
	...
	@echo "30 3 * * * cd $(shell pwd) && sudo bash scripts/enforcement_test.sh >> /var/log/zerotrust-enforcement-test.log 2>&1" >> /tmp/zerotrust_cron.tmp
